    ...
    func testNativeStackKeepsBothCardsWithinSmallAndTallViewports() throws {
        ...
        for (name, width, height, heightLimit) in cases {
            ...
            let frontID = try model.store.add(
                pngData: syntheticPNG(width: width, height: height), pixelWidth: width, pixelHeight: height,
            ...
            let window = NSWindow(
                contentRect: NSRect(origin: CGPoint(x: -3000, y: -3000), size: hostSize),
