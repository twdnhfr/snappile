...
final class MenuPopoverLayoutTests: XCTestCase {
    func testHostingViewKeepsProposedFrameAcrossItemCountTransitions() throws {
        ...
        let controller = NSHostingController(rootView: FixedMenuPopoverView(
        ...
        for itemCount in 0...4 {
            ...
            for proposedHeight in [CGFloat(1), expected.height + 120, CGFloat(900)] {
                let result = measure(controller: controller, model: model, size: expected,
                                     proposedHeight: proposedHeight)
                XCTAssertEqual(result.hostFrame.size, expected,
                ...
                XCTAssertEqual(result.fittingSize.height, expected.height, accuracy: 1,
    ...
    func testEmptyMenuRendersHeaderNearTopEvenAfterOversizedProposal() throws {
        ...
        for y in 0..<bitmap.pixelsHigh {
            for x in 0..<bitmap.pixelsWide {
                ...
                if color.greenComponent > 0.35 && color.greenComponent > color.redComponent*1.6 && color.greenComponent > color.blueComponent*1.08 {
                    firstAccentRow=y; break
                }
            }
    ...
    private func measure(controller: NSHostingController<FixedMenuPopoverView>, model: AppController,
                         size: NSSize, proposedHeight: CGFloat) -> Measurement {
        ...
        return Measurement(hostFrame: host.frame,
