...
final class ThumbnailGeometryTests: XCTestCase {
    ...
    func testPortraitFillCoversSquareAndIsCentered() {
        let rect = DraggableThumbnail.ThumbnailView.drawRect(
            imageSize: NSSize(width: 100, height: 200), in: square, contentMode: .fill)
    ...
    func testLandscapeFillCoversSquareAndIsCentered() {
        let rect = DraggableThumbnail.ThumbnailView.drawRect(
            imageSize: NSSize(width: 200, height: 100), in: square, contentMode: .fill)
    ...
    func testFitContainsPortraitAndLandscape() {
        let portrait = DraggableThumbnail.ThumbnailView.drawRect(
            imageSize: NSSize(width: 100, height: 200), in: square, contentMode: .fit)
        XCTAssertEqual(portrait, NSRect(x: 43.75, y: 0, width: 87.5, height: 175))
        let landscape = DraggableThumbnail.ThumbnailView.drawRect(
            imageSize: NSSize(width: 200, height: 100), in: square, contentMode: .fit)
