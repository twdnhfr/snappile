...
struct SettingsView: View {
    ...
    init(model:AppController) {
        self.model=model; settings=model.settings; store=model.store
    ...
    var body: some View {
        VStack(spacing:0) {
            ...
            Form {
                Section {
                    permissionRow("Bildschirmaufnahme", subtitle:"Für den von dir markierten Bereich", granted:model.screenPermission, action:model.requestScreenPermission)
                ...
                Section {
                    ...
                    HStack(spacing:6) {
                        ...
                        Picker("Taste",selection:$shortcutCode) { ForEach(AppSettings.keys,id:\.code) { Text($0.label).tag($0.code) } }
            ...
            HStack {
                Text("\(store.items.count) Bilder · \(ByteCountFormatter.string(fromByteCount:Int64(store.totalBytes),countStyle:.memory)) PNG-Daten")
    ...
    private func modifierToggle(_ label:String,mask:UInt32) -> some View {
        ...
        } label: { Text(label).font(.system(size:13,weight:.medium)).frame(width:25,height:24) }
            .buttonStyle(.plain)
            .background(shortcutMods & mask != 0 ? pileAccent.opacity(0.17) : Color.primary.opacity(0.04),in:RoundedRectangle(cornerRadius:5))
            .overlay(RoundedRectangle(cornerRadius:5).strokeBorder(shortcutMods & mask != 0 ? pileAccent.opacity(0.5) : Color.primary.opacity(0.08)))
            .accessibilityLabel("Modifier \(label)")
            .accessibilityValue(shortcutMods & mask != 0 ? "Aktiv" : "Inaktiv")
    }
    private func permissionRow(_ title:String,subtitle:String,granted:Bool,action:@escaping()->Void) -> some View {
        HStack {
...
struct MenuPopoverView: View {
    ...
    var body: some View {
        VStack(alignment:.leading,spacing:12) {
            ...
            Button(action:model.beginCapture) {
                HStack { Label("Bereich aufnehmen",systemImage:"viewfinder");Spacer();Text(settings.shortcutLabel).foregroundStyle(.secondary) }.frame(maxWidth:.infinity)
            ...
            if store.items.isEmpty {
                VStack(spacing:6) {
                    ...
                    Text("Bereich aufnehmen, dann direkt\nin deinen KI-Chat ziehen.").font(.system(size:11)).foregroundStyle(.secondary).multilineTextAlignment(.center)
                ...
                ForEach(store.items.prefix(3)) { item in
                    HStack(spacing:10) {
                        ...
                        VStack(alignment:.leading,spacing:3) {
                            ...
                            Text(lifetimeText(item,minutes:settings.expiryMinutes)).font(.system(size:10)).foregroundStyle(.secondary)
                        }
                        Spacer()
            ...
            HStack {
                ...
                Menu {
                    Button("Einrichtung …",action:model.showOnboarding)
                    Button("Ungepinntes löschen",action:model.clearUnpinned).disabled(store.items.allSatisfy(\.isPinned))
                    Divider()
