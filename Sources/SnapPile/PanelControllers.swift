...
final class StatusItemController: NSObject, NSPopoverDelegate {
    ...
    init(model:AppController) {
        self.model=model; item=NSStatusBar.system.statusItem(withLength:NSStatusItem.variableLength)
        hostingController=NSHostingController(rootView:FixedMenuPopoverView(model:model,size:MenuPopoverLayout.size(itemCount:model.store.items.count)))
        super.init()
        if let button=item.button {
            button.image = NSImage(systemSymbolName:"square.3.layers.3d",accessibilityDescription:"SnapPile")
            button.image?.isTemplate=true
            button.target=self;button.action=#selector(toggle(_:));button.sendAction(on:[.leftMouseUp,.rightMouseUp])
    ...
    @objc private func toggle(_ sender:Any?) {
        if NSApp.currentEvent?.type == .rightMouseUp {
            ...
            menu.addItem(withTitle:"SnapPile beenden",action:#selector(NSApplication.terminate(_:)),keyEquivalent:"q")
            close()
            if let button=item.button {
                menu.popUp(positioning:nil,at:NSPoint(x:0,y:button.bounds.minY),in:button)
            }
        } else if popover.isShown { close() }
    ...
    private func makeMenuItem(_ title:String,_ action:Selector)->NSMenuItem { let entry=NSMenuItem(title:title,action:action,keyEquivalent:"");entry.target=self;return entry }
    @objc private func capture() { model.beginCapture() }
    @objc private func showStack() { model.showStack();close() }
...
struct PreviewView:View {
    ...
    let itemID:UUID
    init(model:AppController,itemID:UUID) { self.model=model;store=model.store;self.itemID=itemID }
    var body:some View {
        if let item=store.item(id:itemID) {
            VStack(spacing:0) {
                HStack(spacing:12) {
                    Text("\(item.pixelWidth) × \(item.pixelHeight)").font(.system(size:12,weight:.medium)).monospacedDigit()
                    Text(lifetimeText(item,minutes:model.settings.expiryMinutes)).font(.system(size:11)).foregroundStyle(.secondary)
                    Spacer()
                    Button { model.copy(itemID) } label: { Label("Kopieren",systemImage:"doc.on.doc") }.keyboardShortcut("c",modifiers:.command)
                    Button { model.save(itemID) } label: { Label("Speichern",systemImage:"square.and.arrow.down") }.keyboardShortcut("s",modifiers:.command)
                    SmallIconButton(item.isPinned ? "Pin lösen" : "Anheften",symbol:item.isPinned ? "pin.fill" : "pin",tint:item.isPinned ? pileAccent : .secondary) { store.togglePin(id:itemID) }
        ...
        } else { Text("Dieses Bild ist nicht mehr im Stapel.").foregroundStyle(.secondary) }
    }
}
...
private struct FullImageView:NSViewRepresentable {
    ...
    func makeNSView(context:Context)->NSImageView {
        let view=NSImageView();view.imageScaling = .scaleProportionallyUpOrDown;view.image=NSImage(data:pngData)
