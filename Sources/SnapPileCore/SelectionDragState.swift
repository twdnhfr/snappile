...
struct SelectionDragState {
    ...
    var current: CGRect {
        ...
        return CGRect(x: min(anchor.x, endpoint.x), y: min(anchor.y, endpoint.y),
    ...
    mutating func begin(at point: CGPoint) {
        ...
        anchor = corner; endpoint = corner; isDragging = true
        basePointer = point; baseEndpoint = corner; lastRawPointer = point
    }
    ...
    mutating func update(to rawPoint: CGPoint) {
        ...
        if isMoving {
            ...
            self.endpoint = clamped(CGPoint(x: baseEndpoint.x + rawPoint.x - basePointer.x,
    ...
    mutating func setMoving(_ moving: Bool, at rawPoint: CGPoint) {
        ...
        update(to: rawPoint); isMoving = moving
        basePointer = rawPoint; baseEndpoint = endpoint ?? clamped(rawPoint); lastRawPointer = rawPoint
    }

    mutating func end() { isDragging = false }

    private func clamped(_ rect: CGRect) -> CGRect {
        CGRect(x: min(max(rect.minX, bounds.minX), bounds.maxX - rect.width),
