...
public final class AppSettings: ObservableObject {
    ...
    @Published public var doubleOptionEnabled: Bool { didSet { defaults.set(doubleOptionEnabled, forKey: "doubleOptionEnabled") } }
    @Published public var shortcutKeyCode: UInt32 { didSet { defaults.set(Int(shortcutKeyCode), forKey: "shortcutKeyCode") } }
    @Published public var shortcutModifiers: UInt32 { didSet { defaults.set(Int(shortcutModifiers), forKey: "shortcutModifiers") } }
    @Published public var hasCompletedOnboarding: Bool { didSet { defaults.set(hasCompletedOnboarding, forKey: "hasCompletedOnboarding") } }

    public init(defaults: UserDefaults = .standard) {
        self.defaults = defaults
        defaults.register(defaults: ["expiryMinutes":30, "maxItems":20, "stackSide":"right", "doubleOptionEnabled":true,
                                     "shortcutKeyCode":1, "shortcutModifiers":Int(controlKey | optionKey)])
        expiryMinutes = [5,15,30,60,120].contains(defaults.integer(forKey:"expiryMinutes")) ? defaults.integer(forKey:"expiryMinutes") : 30
        maxItems = [5,10,20,50].contains(defaults.integer(forKey:"maxItems")) ? defaults.integer(forKey:"maxItems") : 20
